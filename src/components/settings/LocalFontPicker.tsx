...
const LOCAL_FONT_FAILURE_MESSAGES: Record<FailureReason, string> = {
  unsupported: '当前浏览器无法列出本机字体，你仍可以输入准确的字体名称。',
  denied: '未获得本机字体权限，你仍可以手动输入字体名称。',
  error: '读取本机字体失败，你仍可以手动输入字体名称。',
};
...
export default function LocalFontPicker({ open, currentFont, returnFocusRef, onOpenChange, onSelect }: LocalFontPickerProps) {
  ...
  const failureMessage = failureReason ? LOCAL_FONT_FAILURE_MESSAGES[failureReason] : '';
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        <DialogHeader className="border-border border-b px-5 py-4 pr-14 text-left">
          <DialogTitle>{'选择本机字体'}</DialogTitle>
          <DialogDescription>{'为文章正文选择这台设备上安装的字体'}</DialogDescription>
        </DialogHeader>
        ...
        {view === 'intro' && (
          <div className="flex flex-col items-center px-6 py-10 text-center">
            ...
            <p className="max-w-sm text-muted-foreground text-sm leading-relaxed">
              {'浏览器会请求读取本机字体列表。字体数据仅用于本地预览和选择，不会上传。'}
            </p>
