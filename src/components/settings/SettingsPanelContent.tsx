...
export default function SettingsPanelContent() {
  ...
  > = {
    ...
    measure: {
      value: measure,
      onApply: setMeasure,
      emptyValue: { label: '自动', fallback: READER_CUSTOM_MEASURE },
    },
  };
  ...
  const renderControl = (item: SettingItem) => {
    ...
    switch (item.type) {
      ...
      case 'switch': {
        ...
        if (!binding) return null;
        return (
          <Switch checked={binding.checked} onCheckedChange={binding.onChange} disabled={disabled} aria-label={item.label} />
        );
      }
    }
  };
                  ...
                  {SECTIONS.map((key) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <span className="relative">{key === 'reader' ? '阅读' : '通用'}</span>
                      </button>
                    );
                  })}
                </div>
                          ...
                          return (
                            <div key={item.key} className="py-2.5 first:pt-1 last:pb-1">
                              <div className="flex items-center justify-between gap-3">
                                <span className={cn('text-sm', disabled && 'opacity-50')}>{item.label}</span>
                              ...
                              {disabled && <p className="mt-1 text-muted-foreground text-xs">{'「减弱动画」开启时不可用'}</p>}
                            </div>
                          );
                        })}
                      </div>
