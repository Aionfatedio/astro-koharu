...
/** Convert folder name to display name (my-awesome-manga -> My Awesome Manga) */
...
async function scanComicFolders(): Promise<ComicFolder[]> {
  ...
  try {
    ...
    for (const entry of entries) {
      ...
      if (images.length > 0) {
        comicFolders.push({ path: folderPath, name: entry.name, images, coverFile, hasManifest });
      }
    }
  } catch (error) {
...
async function generateManifest(folder: ComicFolder, force: boolean): Promise<boolean> {
  ...
  // Preserve custom fields from existing manifest
  const existing = await readExistingManifest(folder.path);
