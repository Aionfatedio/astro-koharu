...
description: 展示目前支持的所有 Markdown 增强功能，包括代码高亮、图片嵌入、视频嵌入等等。
tags:
  - Markdown
  - 示例文章
categories:
...
这是段落中的链接 [新海誠](https://x.com/shinkaimakoto)，不应该被嵌入。
...
<a href="javascript:(function(){
    var imgs = document.querySelectorAll('img[src*=\'pixiv.aionfatedio.top\']');
    imgs.forEach(function(img) {
        var baseUrl = img.src.split('?')[0];
        img.src = baseUrl + '?' + new Date().getTime();
    });
})()">刷新图片</a>  如果没有加载出来请耐心等待哟~ 
