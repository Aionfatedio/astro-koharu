    ...
    <div class="bg-card flex md:flex-col flex-row">
      <div class="flex-1 p-4">
        <div class="mb-2 flex items-center gap-2">
          ${safeLogo ? `<img src="${safeLogo}" alt="" class="link-preview-favicon h-4 w-4 shrink-0" data-domain="${safeDomain}" loading="lazy" aria-hidden="true" />` : ''}
      ...
      ${safeImage ? `<div class="bg-muted relative md:w-full shrink-0 aspect-1200/630 h-38"><img src="${safeImage}" alt="${safeTitle}" class="link-preview-image h-full w-full object-cover" data-fallback-title="${safeTitle}" loading="lazy" /></div>` : ''}
    </div>
  </a>
</div>`;
}
