...
function loadMigrationConfig(raw: string | null): MigrationConfig {
  if (raw === null) return { enableSlugTransliteration: false };

  const parsed = load(raw);
  const config = (parsed && typeof parsed === 'object' ? parsed : {}) as {
    site?: { enableSlugTransliteration?: boolean };
  };
  return { enableSlugTransliteration: config.site?.enableSlugTransliteration === true };
}

function getPathLink(relativePath: string, config: MigrationConfig): string {
  const withoutExtension = relativePath.replace(/\.(md|mdx)$/i, '');
  const legacySlug = withoutExtension
    .split('/')
    .map((segment) => githubSlug(segment))
    .join('/')
    .replace(/\/index$/, '');

  if (!config.enableSlugTransliteration) return legacySlug;
  return slugify(legacySlug, { allowedChars: 'a-zA-Z0-9-_.~/', separator: '-' });
}

function normalizeLegacySlug(slug: string, config: MigrationConfig): string {
  if (!config.enableSlugTransliteration) return slug;
 ...
 * - posts with neither field receive their file path as `link`.
 */
export function planContentMigration(options: ContentMigrationOptions = {}): ContentMigrationPlan {
  ...
  try {
    config = loadMigrationConfig(snapshot.siteConfigOriginal);
  } catch (error) {
  ...
  for (const { sourcePath: filePath, original: raw } of snapshot.files) {
    ...
    if (link && hasSlugField) {
      ...
      targetLink = normalizeLegacySlug(slug, config);
      ...
      targetLink = getPathLink(relativePath, config);
    ...
    const collisionFiles = publicLinks.get(targetLink) ?? [];
    collisionFiles.push(file);
    publicLinks.set(targetLink, collisionFiles);
  ...
  for (const [link, collisionFiles] of publicLinks) {
    if (collisionFiles.length < 2) continue;
    for (const file of collisionFiles) {
      errors.push({ file, message: `存在重复链接 "${link}": ${collisionFiles.join(', ')}` });
    }
  }
