...
test('content migration preserves URLs and is idempotent', () => {
  ...
  try {
    ...
    writePost(contentDir, 'note/path-derived.md', '');
    ...
    assert.equal(first.scannedFiles, 5);
    assert.equal(first.changes.length, 4);
    assert.equal(first.unchangedFiles, 1);
    ...
    assert.equal(second.unchangedFiles, 5);
...
test('directory names remain part of the generated link', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'koharu-migrate-directory-path-'));
  try {
    const configPath = writeSiteConfig(root);
    const contentDir = path.join(root, 'src/content/blog');
    const englishDirectoryFile = writePost(contentDir, 'en/note/foo.md', '');
    const regionDirectoryFile = writePost(contentDir, 'pt-BR/note/foo.md', '');
    ...
    assert.deepEqual(
      ...
      new Map([
        [englishDirectoryFile, 'en/note/foo'],
        [regionDirectoryFile, 'pt-br/note/foo'],
      ]),
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
test('custom links do not influence sibling path-derived links', () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), 'koharu-migrate-sibling-path-'));
  try {
    const configPath = writeSiteConfig(root);
    const contentDir = path.join(root, 'src/content/blog');
    writePost(contentDir, 'note/foo.md', 'link: custom-note\n');
    const archiveFile = writePost(contentDir, 'archive/foo.md', '');

    const plan = runContentMigration({ contentDir, siteConfigPath: configPath });

    assert.deepEqual(plan.errors, []);
    assert.equal(plan.changes.length, 1);
    assert.equal(plan.changes[0]?.sourcePath, archiveFile);
    assert.equal(plan.changes[0]?.link, 'archive/foo');
    assert.equal(matter(fs.readFileSync(archiveFile, 'utf8')).data.link, 'archive/foo');
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
  }
});
...
test('generated links preserve Astro legacy path slugs', () => {
  ...
  try {
    ...
    assert.deepEqual(plan.errors, []);
    assert.deepEqual(
      plan.changes.map(({ sourcePath, link }) => [path.relative(contentDir, sourcePath).replaceAll(path.sep, '/'), link]),
      [
        ['Camel Case.md', 'camel-case'],
        ['Foo Bar/index.md', 'foo-bar'],
        ['en/Nested Folder/Index Post.mdx', 'en/nested-folder/index-post'],
        ['note/Hello, World!.md', 'note/hello-world'],
        ['pt-BR/Release Notes.md', 'pt-br/release-notes'],
      ],
    );
  } finally {
    fs.rmSync(root, { recursive: true, force: true });
...
test('legacy slug fields preserve their pre-migration public URLs', () => {
  ...
  try {
    const configPath = writeSiteConfig(root, true);
    const contentDir = path.join(root, 'src/content/blog');
    writePost(contentDir, 'nested.md', 'slug: en/custom-path\n');
    writePost(contentDir, 'unicode.md', 'slug: 中文路径\n');

    const plan = runContentMigration({ contentDir, siteConfigPath: configPath });

    assert.deepEqual(plan.errors, []);
    assert.equal(matter(fs.readFileSync(path.join(contentDir, 'nested.md'), 'utf8')).data.link, 'en/custom-path');
    assert.equal(matter(fs.readFileSync(path.join(contentDir, 'unicode.md'), 'utf8')).data.link, 'zhong-wen-lu-jing');
...
test('restore replaces user snapshots and migrates restored content', () => {
  ...
  try {
    ...
    fs.writeFileSync(stagedConfig, 'site:\n  enableSlugTransliteration: false\n');
